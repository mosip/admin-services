## MOSIP Admin Services Database scripts inventory and deployment guidelines on postgresql database. 
...
**Example:** the admin-service module script folder is /**mosip_base_directory**>>db_scripts>>mosip_prereg where all the database scripts related to admin-service are available.
...
1) DB Deployment for Admin Service Module databases
...
## DB Deployment for all Admin Service databases with single click deployment:
...
**Step 2** -> Deployment on all admin service databases, run the **"mosip_admin-services_db_deployment.sh"** script which is avialble in the /database directory.  To access **"mosip_admin-services_db_deployment.sh"** script, follow the below given commands:
