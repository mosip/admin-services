### -- ---------------------------------------------------------------------------------------------------------
### -- Script Name		: Hotlist DB Artifacts deploy
### -- Deploy Module 		: MOSIP Hotlist
### -- Purpose    		: To deploy MOSIP Hotlist Database DB Artifacts.       
...
LOG="${LOG_PATH}mosip_hotlist-${today}.log"
touch $LOG
...
#========================================DB Deployment process begins on HOTLIST DB SERVER======================================
...
MASTERCONN=$(PGPASSWORD=$SU_USER_PWD  psql --username=$SU_USER --host=$DB_SERVERIP --port=$DB_PORT --dbname=$DEFAULT_DB_NAME -t -c "select count(1) from pg_roles where rolname IN('hotlistuser')";exit; >> $LOG 2>&1)

if [ ${MASTERCONN} == 0 ]
then
    echo `date "+%m/%d/%Y %H:%M:%S"` ": Creating hotlist database user" | tee -a $LOG 2>&1
    ...
    echo `date "+%m/%d/%Y %H:%M:%S"` ": hotlist user already exist" | tee -a $LOG 2>&1
fi
