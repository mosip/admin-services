...
1. Clone or download the repository as a zip file from [GitHub](https://github.com/mosip/admin-services).
...
2. Run the automation test suite JAR file:
   ```
   java -jar -Dmodules=masterdata -Denv.user=api-internal.<env_name> -Denv.endpoint=<base_env> -Denv.testLevel=smokeAndRegression -jar apitest-masterdata-1.2.3.0-SNAPSHOT-jar-with-dependencies.jar
   ```
...
## 4. **Run the Tests**
   ...
   - In the **Arguments** tab, add the necessary **VM arguments**:
     - **VM Arguments**:
       ```
       -Dmodules=masterdata -Denv.user=api-internal.<env_name> -Denv.endpoint=<base_env> -Denv.testLevel=smokeAndRegression```
...
- **jar**: Specify the name of the JAR file to execute. The version will change according to the development code version. For example, the current version may look like `apitest-masterdata-1.2.3.0-SNAPSHOT-jar-with-dependencies.jar`.
