...
public class MasterdataSearchIntegrationTest {
	...
	public void setup() throws JsonProcessingException {
		...
		/*missingIdDataDtoList.add("30012");
		missingCodeDataDtoList.add("1233");*/

		missingIdDataDtoList.add(new String[] {"30012", null, null});
		missingCodeDataDtoList.add(new String[] {"1233", null, null});
	...
	public void searchInvalidCenterTypeName() throws Exception {
		...
		searchDto.setFilters(Arrays.asList(filter2));
		String validRequest = objectMapper.writeValueAsString(request);
		MvcResult result = mockMvc.perform(post("/registrationcenters/search")
		...
		assertThat(responseWrapper.getErrors().get(0).getErrorCode(), is("KER-MSD-321"));
	}
	...
	@Test
	@WithUserDetails("global-admin")
	public void searchBlockListedWordsTest() throws Exception {
		SearchFilter searchFilter = new SearchFilter();
		...
		blocklistedWords.setWord("BlockListedWord");
		Page<BlocklistedWords> pageContentData = new PageImpl<>(Arrays.asList(blocklistedWords));
		when(masterdataSearchHelper.searchMasterdata(Mockito.eq(BlocklistedWords.class), Mockito.any(), Mockito.any()))
				.thenReturn(pageContentData);
		mockMvc.perform(post("/blocklistedwords/search").contentType(MediaType.APPLICATION_JSON).content(json))
				.andExpect(status().isOk());
	}
	...
	@Test
	@WithUserDetails("global-admin")
	public void filterBlockListedWordsTestForArabicLanguage() throws Exception {
		FilterDto filterDto = new FilterDto();
	...
	@Test
	@WithUserDetails("zonal-admin")
	public void filterBlockListedWordsTestForInvalidLanguageCode() throws Exception {
		FilterDto filterDto = new FilterDto();
	...
	public void searchHolidayByLocationFetchExpTest() throws Exception {
		...
		when(masterdataSearchHelper.searchMasterdata(Mockito.eq(Holiday.class), Mockito.any(), Mockito.any()))
				.thenReturn(pageContentData);
		MvcResult result = mockMvc.perform(post("/holidays/search")
