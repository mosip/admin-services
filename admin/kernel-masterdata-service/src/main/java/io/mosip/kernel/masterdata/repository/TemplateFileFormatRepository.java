...
public interface TemplateFileFormatRepository extends BaseRepository<TemplateFileFormat, String> {
	...
	@Modifying
	@Transactional
	@Query("UPDATE TemplateFileFormat t SET t.updatedBy = ?1,t.isActive = false, t.isDeleted = true , t.deletedDateTime = ?2 WHERE t.code =?3 and (t.isDeleted is null or t.isDeleted = false)")
	...
	@Query("FROM TemplateFileFormat t where t.code = ?1 and (t.isDeleted is null or t.isDeleted = false) and t.isActive = true")
	List<TemplateFileFormat> findAllByCodeAndIsDeletedFalseorIsDeletedIsNull(String code);
