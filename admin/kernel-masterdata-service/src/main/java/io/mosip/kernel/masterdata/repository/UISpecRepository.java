...
public interface UISpecRepository extends BaseRepository<UISpec, String>{
	...
	@Query("FROM UISpec WHERE (isDeleted is null OR isDeleted = false) and id = ?1 and status='DRAFT'")
	...
	UISpec findLatestPublishedUISpec(String domain);
	...
	UISpec findPublishedUISpec(double version, String domain);
